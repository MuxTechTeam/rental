...
function wpestate_get_calendar_allinone ($initial = true, $echo = true) {
    ...
    $month_no       =   1;
    $max_month_no   =   8;//intval   ( wprentals_get_option('wp_estate_month_no_show','') );
    $remaingDays = date("d");
        while ($month_no<$max_month_no){
            $response = wpestate_draw_month_allinone($month_no, $unixmonth, $daywithpost,$thismonth,$thisyear,$last_day ,$remaingDays);
            ...
            $thisyear=$response[1];
            $unixmonth=$response[2];
            $remaingDays=$response[3];
            $month_no++;
        }
...
function    wpestate_draw_month_allinone($month_no, $unixmonth, $daywithpost,$thismonth,$thisyear,$last_day , $remainingDays=date("d")){
    ...
    $myweek = array();
        ...
        $runOnce = 0;
        $day = $remainingDays;
        for ( $counter = $remainingDays; $counter <= $showDays; ++$counter ) {
                if($counter > $daysinmonth and $runOnce == 0){
                     
                    $thismonth = gmdate('m', strtotime('+1 month'));
                    $thisyear  = gmdate('Y', strtotime('+1 month'));
                    $unixmonth = mktime(0, 0 , 0, $thismonth, 1, $thisyear);
                    $last_day = date('t', $unixmonth);
                    $day = 1;
                    $runOnce =1;
      ...
      $calendar_output .= '</div>';
       ...
       // echo "next is...===</br>";
        ...
        //return array($thismonth ,$thisyear ,$unixmonth ,$last_day -$day );
}
