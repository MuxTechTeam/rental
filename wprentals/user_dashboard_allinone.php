...
function wpestate_get_calendar_allinone ($initial = true, $echo = true) {
    ...
    $last_day = $c;
    $month_no       =   1;
    $max_month_no   =  24;// intval   ( wprentals_get_option('wp_estate_month_no_show','') );
    if($c == 30){
        $remaingDays = date("d" , strtotime("-3 days"));
    }else if($c == 31){
        $remaingDays = date("d" , strtotime("-2 days"));
    }else if($c == 28){
        $remaingDays = date("d" , strtotime("-5 days"));
    }
      ...
      $calendar_output .= '</div>';
        ...
        if( !$prop_selection->have_posts() ){
            ...
            while ($prop_selection->have_posts()): $prop_selection->the_post();
                ...
                $calendar_output .= wpestate_draw_month_for_listing($post_id, $daysinmonthToPas, $monthToPas, $yearToPas ,$remainingDays , $dateobj);
            endwhile;
        }
        $calendar_output .= '</div>';
