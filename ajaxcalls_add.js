...
function wpestate_allin_one_owner_insert_book_internal() {
    ...
    function allinone_mark_as_booked(parent, start_date, end_date,curent_id) {
        ...
        /// --- booking form 
        jQuery('#start_date_gust').val(start_date);
        jQuery('#end_date_gust').val(end_date);
    }
...
jQuery('#CustomPricetab').click(function(){
    jQuery("#allinone_set_custom").show();
    jQuery("#allinone_book_for_gust").hide();
});
jQuery('#PainReservationtab').click(function(){
    jQuery("#allinone_set_custom").hide();
    jQuery("#allinone_book_for_gust").show();
});
jQuery('#allinone_reservation_modal').on('hidden.bs.modal', function () {
    // jQuery("#allinone_set_custom").show();
    // jQuery("#allinone_book_for_gust").hide();
    jQuery('#CustomPricetab').trigger('click');
    ...
    jQuery("#bookingPrice").val('');
    
});
jQuery("#checkEmailExist").click(function() {
    if(jQuery('#email_address').val() != ''){
        checkifGustAlreadyRegistered(jQuery('#email_address').val());
    }
});
jQuery("#allinone_book_for_gust").click(function() {
    if(jQuery('#email_address').val() != '' && jQuery('#bookingPrice').val() != ''){
        insertGustBooking();
    }
});

function    checkifGustAlreadyRegistered(email){
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if(data.message == 'email_not_found'){
                ...
                jQuery('#existingGuestID').val(data.user_id);
...
function    insertGustBooking(){
    var   period_extra_price_per_guest, period_price_per_weekeend, period_checkin_change_over, period_checkin_checkout_change_over, period_min_days_booking,start_from, end_to, listing_edit, new_price, ajaxurl;

    start_from      =   jQuery('#start_date_gust').val();
    end_to          =   jQuery('#end_date_gust').val();
    listing_edit    =   jQuery('#property_id').val();
    new_price       =   jQuery('#bookingPrice').val();
    email_address   =   jQuery('#email_address').val();
    firstName       =   jQuery('#firstName').val();
    lastName        =   jQuery('#lastName').val();
    existingGuestID = jQuery('#existingGuestID').val();
    ...
    jQuery.ajax({
        ...
        data: {
            ...
            // 'security'          :   nonce,
            // 'allinone'          :   1
            'action'            :   'wpestate_ajax_add_guest_booking_from_owner',
            'book_from'         :   start_from,
            'book_to'           :   end_to,
            'listing_id'        :   listing_edit,
